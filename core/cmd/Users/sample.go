...
var (
	...
	root = &cobra.Command{
		Use:   "core",
		Short: "Server for XenElectronic Core",
	}
)

func init() {
	cobra.OnInitialize(initConfig)
	root.PersistentFlags().StringVar(&configFile, "config", "", "config file (default is ${HOME}/.xenelectronic-core.yaml")
}
...
func initConfig() {
	...
	if configFile != "" {
		...
		viper.SetConfigName(".xenelectronic-core.yaml")
	}

	// enable environment vars
	viper.SetEnvPrefix("XENELECTRONIC_CORE")
